...
test('bundled startup navigation is complete, unique and uncapped', () => {
  ...
  assert.equal(bootstrap.items.filter((item) => item.categoryKeys?.includes('iranian-series')).length, 18);
...
test('startup, resume and manual refresh never download the detail-rich full index', () => {
  ...
  assert.ok(reload.includes('await loadBootstrapContent()'));
