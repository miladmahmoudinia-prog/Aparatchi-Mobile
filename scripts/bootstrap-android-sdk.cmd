...
if not exist "%PLATFORM_TARGET%\android.jar" (
  call :download_platform
  if errorlevel 1 exit /b !errorlevel!
)

if not exist "%BUILD_TARGET%\aapt2.exe" (
  call :reuse_build_tools
  if errorlevel 1 exit /b !errorlevel!
)
if not exist "%BUILD_TARGET%\aapt2.exe" (
...
set "PLATFORM_JAR="
for /f "delims=" %%F in ('where /r "!PLATFORM_TMP!" android.jar 2^>nul') do if not defined PLATFORM_JAR set "PLATFORM_JAR=%%F"
if not defined PLATFORM_JAR exit /b 36
...
set "AAPT2_FILE="
for /f "delims=" %%F in ('where /r "!BUILD_TMP!" aapt2.exe 2^>nul') do if not defined AAPT2_FILE set "AAPT2_FILE=%%F"
if not defined AAPT2_FILE exit /b 46
...
if exist "!ZIP_PATH!" (
  ...
  if "!ZIP_OK!"=="0" del /q "!ZIP_PATH!"
)
if not exist "!ZIP_PATH!" type nul > "!ZIP_PATH!"
echo Preparing !ZIP_LABEL!...
...
robocopy "!COPY_SRC!" "!COPY_DST!" /E /NFL /NDL /NJH /NJS /NP
if errorlevel 8 exit /b 72
exit /b 0
