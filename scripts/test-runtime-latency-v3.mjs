...
assert(stable.includes('const rawPromise = firstValidStableDetailPath(rawCandidates, summary);'), 'Raw pointer request is not started concurrently');
assert(stable.includes('const mirrorPromise = firstValidStableDetailPath(mirrorCandidates, summary);'), 'CDN pointer request is not started concurrently');
assert(stable.includes('Promise.race(['), 'Stable pointer still waits on sequential mirrors');
assert(stable.includes('setTimeout(() => resolve(null), 450)'), 'Raw source preference is not bounded');
assert(!stable.includes('for (const candidate of candidates)'), 'Stable pointer mirrors are still sequential');
...
console.log(JSON.stringify({
  ...
  stablePointer: 'raw-and-cdn-concurrent-with-bounded-raw-preference',
